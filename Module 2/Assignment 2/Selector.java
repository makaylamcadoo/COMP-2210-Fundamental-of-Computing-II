 ...
 *
 * @author  MAKAYLA MCADOO (mdm0114@auburn.edu)
 *
 */
...
*/
   ...
   public static <T> T kmin(Collection<T> coll, int k, Comparator<T> comp) {
      ...
      ArrayList<T> newColl = new ArrayList<T>(coll);
      java.util.Collections.sort(newColl, comp);
      Iterator<T> itr = newColl.iterator();
      itr.next();
      ...
      for (int i = 0; i < k - 1; i++) {
         if (newColl.get(i) == newColl.get(i+1)) {
            k++;
         }
      }
      T kmin = newColl.get(k - 1);
      return kmin;
   ...
   public static <T> T kmax(Collection<T> coll, int k, Comparator<T> comp) {
      ...
      T kmax = newColl.get(coll.size() - k);
      return kmax;
